...
import DesktopApp from '../desktop-apps/DesktopApp';
...
const Window = ({ appID }: WindowProps) => {
  const { activeApp, activeAppZIndex, setActiveApp } = useAppsStore(
    useShallow((state) => ({
  ...
  useEffect(() => {
    if (activeApp === appID) setAppZIndex(activeAppZIndex + 2);
  ...
  const { resizable, height, width, title } = appsConfig[appID];
  ...
  return (
    ...
    >
      ...
      >
        <div className={clsx(styles.titleBar, 'app-window-drag-handle')}>
...
const useMaximizeWindow = (windowRef: RefObject<WindowRnd>) => {
  ...
  const transitionClearanceRef = useRef<number>(0);
...
// Don't memoize Window since it needs to react to activeApp changes from Zustand store
